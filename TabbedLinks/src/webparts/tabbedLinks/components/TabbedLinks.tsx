...
import styles from './TabbedLinks.module.scss';
import { ITabbedLinksProps } from './ITabbedLinksProps';
import { ITabbedLinksState } from './ITabbedLinksState';
...
export default class TabbedLinks extends React.Component<ITabbedLinksProps, ITabbedLinksState> {
  constructor(props) {
    super(props);
    this.state = { 
      showPanel: false,
  ...
  public render(): React.ReactElement<ITabbedLinksProps> {
    ...
    class SingleLink extends React.Component<{cardContents, url, iconName, name, openInNewTab, isEditModetmp, headingKey, linkKey, _editLink:(headingIndex:number, linkIndex:number, iconName: string) => void, _moveLink:(cardContents, moveDown:boolean,headingIndex:number, linkIndex:number)=> void, _deleteLink:(cardContents, headingIndex:number, linkIndex:number)=> void}> {
      ...
      public render() {
        if(this.props.isEditModetmp)
        {
          ...
          return (
            <div className={`ms-Grid-row ${styles.hoverBorder}`}>
              <div className="ms-Grid-col ms-lg8 ms-md6 ms-sm4">
...
class EditHeadingPanel extends React.Component<{cardContents, isNewItem, headingIndex, _onCloseHeadingPanel: () => void, _headingSave(configOptionstmp, isNewItem:boolean, headingKey: number, headingValue: string): void}, any> {
    ...
    // START Tabbed Menu
    // class SingleTab extends React.Component<{ tab, currentIndex}> {
    // public render(){
    //   let currentTabItems = this.props.tab.links;
    //   let tmpItems = [];
    //   for (var i = 0; i < currentTabItems.length; i++) {
    //     var tmpItemProps = {
    //       key: 'Tab' + this.props.currentIndex + '_link' + i,
    //       name: this.props.tab.links[i].name,
    //       icon: this.props.tab.links[i].iconName,
    //       href: this.props.tab.links[i].link,
    //       target: this.props.tab.links[i].openInNewTab == true ? '_blank' : '_self'
    //       }
    //     tmpItems.push(tmpItemProps);
    //     }
    ...
    // }
    // }      
    // End Single Tabbed menu
    let allTabs = JSON.parse(this.props.menuConfig).cards;
    var tmpCommandBarItems = []
    for (var i = 0; i < allTabs.length; i++) {
      var tmpCommandBarSubItems = []
      for (var j = 0; j < allTabs[i].links.length; j++) {
        var commandBarSubItem = {
          key: 'Tab' + i + '_link' + j,
          name: allTabs[i].links[j].name,
          icon: allTabs[i].links[j].iconName,
          href: allTabs[i].links[j].link,
          target: allTabs[i].links[j].openInNewTab == true ? '_blank' : '_self'
          }
          tmpCommandBarSubItems.push(commandBarSubItem)
      }        
      var commandBarItem = {
        key: 'Tab' + i,
        name: allTabs[i].heading,
        className: 'ms-CommandBarItem',        
        subMenuProps: {
          items:tmpCommandBarSubItems
          }
        }
        tmpCommandBarItems.push(commandBarItem);
      }
    
    // let allLinksInTabs = allTabs.map((tab, index) =>
    // // Correct! Key should be specified inside the array.
    // <SingleTab tab={tab} currentIndex={index} />    
    // );

    return (
      <div className={styles.tabbedLinks}>
       <CommandBar
          isSearchBoxVisible={ false }
          items={tmpCommandBarItems}
        />
      ...
      {_isEditMode &&
      <div>
        <MessageBar messageBarType={MessageBarType.info} className={styles.marginTop10px} >
          <ul>
              <li><strong>Note: </strong> This webpart will be hidden after refresh on 'Read' view and only Menu will be visible.</li>
              <li>Click on Configure to start configuring your menu</li>
              <li>Optionally, you will be able to copy configuration text from/ to 'properties' to any other page.</li>
              <li>If you don't want this Tabbed link, delete this webpart.</li>
          </ul>
          <PrimaryButton  iconProps={ { iconName: 'settings' } } onClick={ () => this.setState({ showPanel: true }) } >Configure</PrimaryButton>
        </MessageBar>
        <Panel
